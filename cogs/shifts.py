...
# ==================== БЛОК НАСТРОЕК СМЕН ====================
# ИЗМЕНЕНИЕ: Для 'ad' и '24ad' теперь установлены абсолютно одинаковые ID ролей
CONFIG = {
    "bm": {
    ...
    "ad": {
        "role_id": 1430913711979233312,        # Общая роль сотрудника
        "curator_role_id": 1446784416352440493,   # Общая роль куратора
        "slots": [
...
async def get_shift_interface(shift_type: str, guild: discord.Guild, target_date=None):
    ...
    # ИЗМЕНЕНИЕ: Цвета эмбедов (#7BFE6C -> 0x7BFE6C) и новые заголовки
    if shift_type == "24ad":
    ...
    # ИЗМЕНЕНИЕ: Проверка на день директора перенесена на целевую дату (актуально для автоотправки)
    if shift_type == "24ad" and target_date.weekday() == 6:
        ...
        embed.set_footer(text=f"Дата: {date_str}")
        view = ShiftView(shift_type, occupied_slots={s: 0 for s in slots})
        return embed, view

    embed = discord.Embed(title=title_text, color=color_val)
    embed.set_footer(text=f"Дата: {date_str}") # Умное сохранение даты в метаданные сообщения
...
class ShiftSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # Читаем целевую дату из футера эмбеда
        footer_text = interaction.message.embeds[0].footer.text
        target_date_str = footer_text.replace("Дата: ", "").strip()
...
class CuratorRemoveByIdModal(discord.ui.Modal, title="Принудительное снятие"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        footer_text = self.parent_message.embeds[0].footer.text
        target_date_str = footer_text.replace("Дата: ", "").strip()
        t_date = datetime.strptime(target_date_str, "%Y-%m-%d").date()
...
# ==================== МОДАЛКА КУРАТОРА: ПЕРЕДАТЬ (2 ПОЛЯ) ====================
class CuratorTransferModal(discord.ui.Modal, title="Кураторская передача смены"):
    from_input = discord.ui.TextInput(label="ID сотрудника, у кого снять смену", placeholder="Discord ID цифрами...", required=True, min_length=15, max_length=21)
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            cursor = await db.execute("SELECT slot FROM shifts WHERE date = ? AND department = ? AND user_id = ?", (target_date_str, self.shift_type, from_id))
            row = await cursor.fetchone()
            if not row:
            ...
            cursor = await db.execute("SELECT 1 FROM shifts WHERE date = ? AND department = ? AND user_id = ?", (target_date_str, self.shift_type, to_id))
            if await cursor.fetchone():
            ...
            await db.execute(
                ...
                (to_id, target_date_str, self.shift_type, from_id)
            )
            await db.commit()
...
class ShiftView(discord.ui.View):
    def __init__(self, shift_type: str, occupied_slots: dict = None, register_all: bool = False):
        ...
        # ИЗМЕНЕНИЕ: Теперь абсолютно ВСЕ отделы получают удобный выпадающий список
        free_slots = [s for s in slots if s not in occupied]
        self.add_item(ShiftSelect(shift_type, free_slots, register_all))
        
        # Кнопки ложатся на 1-ю строку (список автоматически занимает строку 0)
...
class ShiftCog(commands.Cog):
    ...
    # ИЗМЕНЕНИЕ: Обновлен заголовок и цвет (#B26CFE) для команды Дня Директора
    @commands.command()
    ...
    # ИЗМЕНЕНИЕ: Добавлен аргумент day_offset для легкой реализации автоотправки на завтра
    @commands.command()

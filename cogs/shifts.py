...
CONFIG = {
    "bm": {
        ...
        "slots": ["07:00", "15:00", "23:00"]  # Старая сетка для BM (не трогаем)
    },    
    "ad": {
        "role_id": 1430913711979233312,        # НАСТРОЙКА: Роль сотрудников AD и 24AD (ОДИНАКОВАЯ)
        "curator_role_id": 1446784416352440493,   # НАСТРОЙКА: Роль кураторов AD и 24AD (ОДИНАКОВАЯ)
        "slots": [
        ...
        ] # 12 смен с часу дня до часу ночи
    },    
    "24ad": {
        "role_id": 1430913711979233312,        # НАСТРОЙКА: Дублируем ту же роль, что и в AD
        "curator_role_id": 1446784416352440493,   # НАСТРОЙКА: Дублируем ту же роль куратора, что и в AD
        "slots": [
...
async def process_booking(interaction: discord.Interaction, shift_type: str, slot_time: str):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        # Проверка на дубликат (одна смена в день)
        cursor = await db.execute(
...
class CuratorRemoveModal(discord.ui.Modal, title="Принудительное снятие"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        # Умный поиск: если куратор ввел только "14:00", бот сам найдет "14:00 - 14:59"
        target_slot = None
        for s in slots:
            if raw_slot in s:
                target_slot = s
...
class ShiftView(discord.ui.View):
    def __init__(self, shift_type: str, occupied_slots: dict = None, register_all: bool = False):
        ...
        # Динамическое разделение логики Кнопки VS Меню выбора
        if shift_type == "24ad":
            free_slots = [s for s in slots if s not in occupied]
            self.add_item(ShiftSelect(shift_type, free_slots, register_all))
            row_idx = 1
        else:
            row_idx = 3 # Переносим кнопки управления вниз, чтобы не мешать кнопкам времени
            if register_all:
                for slot in slots:

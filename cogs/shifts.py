...
async def get_shift_interface(shift_type: str, guild: discord.Guild, target_date=None):
    ...
    # ИЗМЕНЕНИЕ: Футеры полностью удалены из всех эмбедов
...
# Вспомогательная функция для получения даты сообщения из БД
async def fetch_date_by_msg(message_id: int) -> str:
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        return row[0] if row else str(date.today())
...
class ShiftSelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        # ИЗМЕНЕНИЕ: Дата берется скрытно из БД по ID сообщения
...
class ShiftView(discord.ui.View):
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        ...
        if "p_ctrl_cancel" in custom_id or "p_ctrl_transfer" in custom_id:
            ...
            if not row:
                await interaction.response.send_message("❌ У вас нет active смены в этом отделе на этот день.", ephemeral=True)
                return False
...
class ShiftCog(commands.Cog):
    ...
    async def cog_load(self):
        async with aiosqlite.connect(DB_NAME) as db:
            ...
            """)
            # ИЗМЕНЕНИЕ: Новая таблица связки Сообщение -> Дата панели
            await db.execute("""
    ...
    async def start_shifts(self, ctx, shift_type: str, day_offset: int = 0):
        ...
        # ИЗМЕНЕНИЕ: Сразу после отправки логируем панель в базу данных
        async with aiosqlite.connect(DB_NAME) as db:
            await db.execute(
